...
//            }
        ...
        try {
            ...
            try {
                if (prop.getProperty("AKTIFTOKO").equals("cafe")) {
                    kode_toko.setText(prop.getProperty("AKTIFTOKO"));
//                    BtnToko.setEnabled(false);
                    kode_toko.setEnabled(false);
                }
            } catch (Exception e) {
                System.out.println("Notif Aktif Toko : " + e.toString());
    ...
    private void initComponents() {
        ...
        label2.setText("Lokasi :");
        label2.setName("label2"); // NOI18N
        FormInput.add(label2);
        label2.setBounds(230, 10, 70, 23);
        ...
        kode_toko.setName("kode_toko"); // NOI18N
        kode_toko.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                kode_tokoKeyPressed(evt);
            }
        });
        FormInput.add(kode_toko);
        kode_toko.setBounds(310, 10, 140, 23);

        label8.setText("Harga Dasar : Rp.");
        label8.setName("label8"); // NOI18N
        FormInput.add(label8);
        label8.setBounds(450, 10, 120, 23);
    ...
    private void BtnSimpanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSimpanActionPerformed
        if (kode_brng.getText().trim().equals("")) {
            ...
            if (Sequel.menyimpantf("tokobarang", "?,?,?,?,?,?,?,?,?,?,'1',?", "Kode Barang", 11, new String[]{
                kode_brng.getText(), nama_brng.getText(), kode_sat.getText(), kdjenis.getText(), stok.getText(),
                dasar.getText(), beli.getText(), distributor.getText(), grosir.getText(), retail.getText(), kode_toko.getText()
            }) == true) {
                tampil();
                emptTeks();
            }
    ...
    private void BtnEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnEditActionPerformed
        if (kode_brng.getText().trim().equals("")) {
            ...
            if (tbJnsPerawatan.getSelectedRow() > -1) {
                if (Sequel.mengedittf("tokobarang", "kode_brng=?", "kode_brng=?,nama_brng=?,kode_sat=?,jenis=?,stok=?,dasar=?,h_beli=?,distributor=?,grosir=?,retail=?,lokasi=?", 12, new String[]{
                    kode_brng.getText(), nama_brng.getText(), kode_sat.getText(), kdjenis.getText(), stok.getText(), dasar.getText(), beli.getText(), distributor.getText(), grosir.getText(), retail.getText(), tbJnsPerawatan.getValueAt(tbJnsPerawatan.getSelectedRow(), 0).toString(), kode_toko.getText()
                }) == true) {
                    tampil();
                    emptTeks();
                }
...
//        akses.setform("TokoBarang");
    ...
    private void kode_tokoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kode_tokoKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_kode_tokoKeyPressed
    ...
    private widget.TextBox kode_toko;
        ...
        try {
            if (TCari.getText().trim().equals("")) {
                ps = koneksi.prepareStatement(
                        ...
                        + "from tokobarang inner join kodesatuan on tokobarang.kode_sat=kodesatuan.kode_sat "
                        + "inner join tokojenisbarang on tokobarang.jenis=tokojenisbarang.kd_jenis "
                        + "where tokobarang.status='1' and tokobarang.lokasi like '%" + kode_toko.getText() + "%' order by tokobarang.kode_brng");
            } else {
                ps = koneksi.prepareStatement(
                        "select tokobarang.kode_brng,tokobarang.nama_brng,kodesatuan.satuan,tokojenisbarang.nm_jenis, "
                        + "tokobarang.stok,tokobarang.dasar,tokobarang.h_beli,tokobarang.distributor,tokobarang.grosir,tokobarang.retail "
                        + "from tokobarang inner join kodesatuan on tokobarang.kode_sat=kodesatuan.kode_sat "
                        + "inner join tokojenisbarang on tokobarang.jenis=tokojenisbarang.kd_jenis "
                        + "where tokobarang.status='1' and tokobarang.lokasi like '%" + kode_toko.getText() + "%' and tokobarang.kode_brng like ? "
                        + "or tokobarang.status='1' and tokobarang.lokasi like '%" + kode_toko.getText() + "%' and tokobarang.nama_brng like ? "
                        + "or tokobarang.status='1' and tokobarang.lokasi like '%" + kode_toko.getText() + "%' and kodesatuan.satuan like ? "
                        + "or tokobarang.status='1' and tokobarang.lokasi like '%" + kode_toko.getText() + "%' and tokojenisbarang.nm_jenis like ? order by tokobarang.kode_brng");
            }
    ...
    private void getData() {
        if (tbJnsPerawatan.getSelectedRow() != -1) {
            ...
            kode_sat.setText(Sequel.cariIsi("select kode_sat from tokobarang where kode_brng=? and lokasi like'%" + kode_toko.getText() + "%'", kode_brng.getText()));
            kdjenis.setText(Sequel.cariIsi("select jenis from tokobarang where kode_brng=? and lokasi like'%" + kode_toko.getText() + "%'", kode_brng.getText()));
        }
    }
