        ...
        tokomember.addWindowListener(new WindowListener() {
            ...
            public void windowClosed(WindowEvent e) {
                if (tokomember.getTable().getSelectedRow() != -1) {
                    ...
                    sisapiutang = Sequel.cariIsiAngka("SELECT ifnull(SUM(tokopiutang.sisapiutang),0) FROM tokopiutang where tokopiutang.no_member=? and tokopiutang.toko like " + toko + "", Kdmem.getText())
                    ...
                    Sequel.cariIsi("select nota_piutang from tokopiutang where no_member=? and tokopiutang.toko like " + toko + " order by tgl_piutang desc limit 1", NoNota, Kdmem.getText());
                }
            }
        ...
        if (NoNota.getText().trim().equals("")) {
            ...
            if (Sequel.cariInteger("select count(nota_piutang) from tokopiutang where nota_piutang=?", NoNota.getText()) > 0) {
                ...
                if (Sequel.menyimpantf("toko_bayar_piutang", "?,?,?,?,?,?,?", "Pembayaran", 7, new String[]{
                    ...
                    sukses = jur.simpanJurnal(NoNota.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""), "U", "BAYAR PIUTANG TOKO" + ", OLEH " + Toko.nip);
                } else {
                    sukses = false;
                }
        ...
        if (NoNota.getText().trim().equals("")) {
            ...
            if (tbKamar.getSelectedRow() > -1) {
                ...
                if (Sequel.queryu2tf("delete from toko_bayar_piutang where tgl_bayar=? and no_member=? and nota_piutang=? and kd_rek=? and kd_rek_kontra=?", 5, new String[]{
                    ...
                    sukses = jur.simpanJurnal(NoNota.getText(), Valid.SetTgl(Tanggal.getSelectedItem() + ""), "U", "PEMBATALAN BAYAR PIUTANG TOKO" + ", OLEH " + Toko.nip);
                } else {
                    sukses = false;
                }
...
private void ppNotaPiutangBtnPrintActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppNotaPiutangBtnPrintActionPerformed
    ...
    if (tabMode.getRowCount() == 0) {
        ...
        if (tbKamar.getSelectedRow() > -1) {
            ...
            Valid.panggilUrl("billing/LaporanPiutang.php?petugas=" + Toko.nip.replaceAll(" ", "_") + "&tanggal=" + Tanggal.getSelectedItem().toString().replaceAll(" ", "_"));
        }
    }
    this.setCursor(Cursor.getDefaultCursor());
...
private void BtnSeekActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnSeekActionPerformed
    ...
    private void BtnCari1ActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BtnCari1ActionPerformed
        if (NoNota.getText().trim().equals("")) {
            ...
            if (Sequel.cariInteger("select count(nota_piutang) from tokopiutang where nota_piutang=? and tokopiutang.toko like " + toko + "", NoNota.getText()) > 0) {
                Kdmem.setText(Sequel.cariIsi("select no_member from tokopiutang where nota_piutang=? and tokopiutang.toko like " + toko + "", NoNota.getText()));
                Nmmem.setText(Sequel.cariIsi("select nm_member from tokopiutang where nota_piutang=? and tokopiutang.toko like " + toko + "", NoNota.getText()));
                sisapiutang = Sequel.cariIsiAngka("SELECT ifnull(SUM(tokopiutang.sisapiutang),0) FROM tokopiutang where tokopiutang.nota_piutang=? and tokopiutang.toko like " + toko + "", NoNota.getText())
                        - Sequel.cariIsiAngka("SELECT ifnull(SUM(toko_bayar_piutang.besar_cicilan),0) FROM toko_bayar_piutang where toko_bayar_piutang.nota_piutang=?", NoNota.getText());
