        ...
        if (NoFaktur.getText().trim().equals("")) {
            ...
            if (reply == JOptionPane.YES_OPTION) {
                    ...
                    for (i = 0; i < jml; i++) {
                        ...
                        if (Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()) > 0) {
                            if (Sequel.menyimpantf2("toko_detail_beli", "?,?,?,?,?,?,?,?,?", "Transaksi Pengadaan", 9, new String[]{
                                ...
                                Trackbarang.catatRiwayat(tbDokter.getValueAt(i, 1).toString(), Valid.SetAngka(tbDokter.getValueAt(i, 0).toString()), 0, "Pengadaan", Toko.nip, "Simpan");
                                ...
                                if (tbDokter.getValueAt(i, 4).toString().equals("true") && (!DaftarMenu.TBARANG.equals(""))) {
                ...
                if (sukses == true) {
                    ...
                    sukses = jur.simpanJurnal(NoFaktur.getText(), Valid.SetTgl(TglBeli.getSelectedItem() + ""), "U", "PEMBELIAN BARANG TOKO" + ", OLEH " + Toko.nip);
                }
...
private void tbDokterKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tbDokterKeyPressed
    ...
    private void tampil() {
        ...
        try {
            ...
            ps = koneksi.prepareStatement(
                    ...
                    + " from tokobarang where tokobarang.status='1' and tokobarang.kode_brng like ? and tokobarang.lokasi like " + toko + " or "
                    + " tokobarang.status='1' and tokobarang.nama_brng like ? and tokobarang.lokasi like " + toko + " or "
                    + " tokobarang.status='1' and tokobarang.jenis like ? and tokobarang.lokasi like " + toko + " order by tokobarang.nama_brng");
            try {
    ...
    private void autoNomor() {
        Valid.autoNomer3("select ifnull(MAX(CONVERT(RIGHT(no_faktur,3),signed)),0) from tokopembelian where tgl_beli='" + Valid.SetTgl(TglBeli.getSelectedItem() + "") + "'", "PT" + TglBeli.getSelectedItem().toString().substring(6, 10) + TglBeli.getSelectedItem().toString().substring(3, 5) + TglBeli.getSelectedItem().toString().substring(0, 2), 3, NoFaktur);
    }
