...
public class TokoPenjualan extends javax.swing.JDialog {
    ...
    public TokoPenjualan(java.awt.Frame parent, boolean modal) {
        ...
        kdtoko.setText(frmUtama.IDTOKO);
    ...
    private void initComponents() {
        ...
        BtnBaru.setIcon(new javax.swing.ImageIcon(getClass().getResource("/picture/New.png"))); // NOI18N
        BtnBaru.setMnemonic('S');
        BtnBaru.setText("Baru");
        BtnBaru.setToolTipText("Alt+S");
        ...
        CmbAkun.setBounds(449, 70, 190, 23);

        label17.setText("Member :");
        label17.setName("label17"); // NOI18N
        label17.setPreferredSize(new java.awt.Dimension(60, 23));
        panelisi3.add(label17);
        label17.setBounds(365, 10, 80, 23);

        kdtoko.setName("kdtoko"); // NOI18N
        kdtoko.setPreferredSize(new java.awt.Dimension(80, 23));
        kdtoko.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyPressed(java.awt.event.KeyEvent evt) {
                kdtokoKeyPressed(evt);
            }
        });
        panelisi3.add(kdtoko);
        kdtoko.setBounds(690, 70, 130, 23);
...
//            tbObat.requestFocus();
            ...
            param.put("nota_jual", NoNota.getText());
            param.put("kembali", LKembali.getText());
            param.put("logo", Sequel.cariGambar("select logo from setting"));
            Valid.MyReportqry("rptNotaToko.jasper", "report", "::[ Data Nota Toko ]::",
                    "SELECT tokomember.no_member,"
                    + "tokomember.nama,"
                    + "tokomember.alamat,"
                    ...
                    + "WHERE "
                    + "tokopenjualan.nota_jual = '" + NoNota.getText() + "'", param);
            this.setCursor(Cursor.getDefaultCursor());
//            this.setCursor(Cursor.getDefaultCursor());
...
private void ppBersihkanActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_ppBersihkanActionPerformed
    ...
    private void kdtokoKeyPressed(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_kdtokoKeyPressed
        // TODO add your handling code here:
    }//GEN-LAST:event_kdtokoKeyPressed
    ...
    private void tampil() {
        ...
        try {
            ps = koneksi.prepareStatement(
                    ...
                    + "where tokobarang.stok>0 and tokobarang.status='1' and tokobarang.lokasi like '%" + kdtoko.getText().trim() + "%' and "
                    + "(tokobarang.kode_brng like " + cari + " or tokobarang.nama_brng like " + cari + " or tokojenisbarang.nm_jenis like " + cari + ") order by tokobarang.nama_brng");
            try {
//                ps.setString(1, "'%" + kdtoko.getText().trim() + "%'");
...
//        }
    ...
    private void isSimpan() {
        for (i = 0; i < tabMode.getRowCount(); i++) {
            if (Valid.SetAngka(tabMode.getValueAt(i, 0).toString()) > 0) {
                if (Sequel.menyimpantf2("toko_detail_jual", "?,?,?,?,?,?,?,?,?,?,?", "Barang", 11, new String[]{
                    ...
                    System.out.println("toko_detail_jual");
                    Trackbarang.catatRiwayat(tabMode.getValueAt(i, 1).toString(), 0, Valid.SetAngka(tabMode.getValueAt(i, 0).toString()), "Penjualan", Toko.nip, "Simpan");
                    Sequel.mengedit("tokobarang", "kode_brng=?", "stok=stok-?", 2, new String[]{
                        tbObat.getValueAt(i, 0).toString(), tbObat.getValueAt(i, 1).toString()
                    });
                    System.out.println("tokobarang");
                } else {
                    sukses = false;
                }
            }
        ...
        if (sukses == true) {
            ...
            sukses = jur.simpanJurnal(NoNota.getText(), Valid.SetTgl(Tgl.getSelectedItem() + ""), "U", "PENJUALAN TOKO / MINIMARKET / KOPERASI, OLEH " + Toko.nip);

            System.out.println("tampjurnal");
        }
    }
